...
| Track | Status |
|--|--|
| QEMU/UTM virt | **Regression freeze** M33–M39 (`Notes.ZC` / DocLib `#include`) still the bar — see ACCEPTANCE.md. **Lattice compose** continued on top (M151–M180: FB teardown, `latticeplay`, controls cues, DrawIt head, depth reset, `Lattice()` entry, ~60 Hz idle). Scripted QEMU ≠ live UTM keyboard/tablet proof. |
